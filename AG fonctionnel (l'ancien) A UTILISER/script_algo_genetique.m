...
liste_taub_max_par_nbre_profils=zeros(20,1);
liste_taub_moyen_par_nbre_profils=zeros(20,1);

for n=1:20
    [liste_taub_max,~,liste_taub_moyen,~,~,iteration]=test_algo_genetique(n,500);
    liste_taub_max_par_nbre_profils(n,1)=liste_taub_max(iteration);
    liste_taub_moyen_par_nbre_profils(n,1)=liste_taub_max(iteration);
end

n=1:1:20;
figure(1);hold on;
title('Tau-b maxi et moyen, fonction du nombre profils par individu, 30 alternatives'); hold on;
